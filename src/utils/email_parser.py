...
class EmailParser:
    ...
    def parse_headers(msg: Message) -> Dict:
        ...
        try:
            ...
            if to_header:
                recipients.extend([addr[1] for addr in email.utils.getaddresses([to_header])])
            
            # 解析抄送
            cc = []
            cc_header = msg.get('Cc', '')
            if cc_header:
                cc.extend([addr[1] for addr in email.utils.getaddresses([cc_header])])
            
            # 解析密送
            bcc = []
            bcc_header = msg.get('Bcc', '')
            if bcc_header:
                bcc.extend([addr[1] for addr in email.utils.getaddresses([bcc_header])])
                ...
                for part in msg.walk():
                    ...
                    if content_type == 'text/plain' and 'attachment' not in content_disposition:
                        ...
                        if payload:
                            ...
                            try:
                                content_text += payload.decode(charset, errors='ignore')
                            except:
                        ...
                        if payload:
                            charset = part.get_content_charset() or 'utf-8'
                            try:
                                content_html += payload.decode(charset, errors='ignore')
                            except:
                        ...
                        if filename:
                            # 解码文件名
                            decoded_filename = EmailParser._decode_filename(filename)
            ...
            if msg.is_multipart():
                for part in msg.walk():
                    ...
                    if 'attachment' in content_disposition or part.get_filename():
                        ...
                        if part_filename:
                            decoded_filename = EmailParser._decode_filename(part_filename)
