        ...
        try:
            async with self.db_manager.get_transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    logger.info(f"邮件保存成功: ID={email_id}, message_id={email.message_id}")
                    return email_id
        ...
        try:
            async with self.db_manager.get_transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    logger.info(f"附件信息保存成功: ID={attachment_id}, 文件={attachment.original_filename}")
        ...
        try:
            async with self.db_manager.get_transaction() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if existing:
                        email_id = existing[0]
                        logger.debug(f"邮件已存在: ID={email_id}, message_id={email.message_id}")
                    ...
                    logger.info(f"邮件及附件保存完成: email_id={email_id}, 附件数={len(attachment_ids)}")
